...
router.post("/webhook", async (req, res) => {
  try {
    ...
    // 4. Trích xuất username từ đúng các mẫu nội dung QR mà frontend tạo ra.
    const match = content
  ? content.match(/([A-Za-z0-9_]+)\s*[-:]\s*CHUC\s+MUNG\s+SINH\s+NHAT/i)
  : null;
...
router.post("/topup", requireAuth, async (req, res) => {
  ...
  if (amountNum > MAX_TOPUP_AMOUNT) {
    return res.status(400).json({
      error: `Số tiền vượt quá giới hạn (${MAX_TOPUP_AMOUNT.toLocaleString("vi-VN")}đ).`,
    });
  }
