 ...
 * Chạy: node seed.js
 *
 * - Tạo danh sách acc mẫu vào data/accounts.json
 * - Tạo 2 tài khoản mặc định vào data/users.json: admin/admin123 và user/user123
 *   (mật khẩu đã mã hoá, số dư ví bắt đầu ở 0đ)
 * - Tạo file rỗng data/purchases.json cho lịch sử mua hàng
 *
 * An toàn khi chạy nhiều lần: nếu đã có dữ liệu, script sẽ không ghi đè
 * trừ khi truyền --force
 */
const fs = require("fs");
...
function hasExistingData() {
  try {
    ...
    return (accounts && accounts.length > 0) || (users && users.length > 0);
  } catch (err) {
    return false;
  }
}
...
if (hasExistingData() && !force) {
  console.log("⚠️  data/accounts.json hoặc data/users.json đã có dữ liệu. Dùng `node seed.js --force` nếu muốn ghi đè.");
  process.exit(0);
}
...
async function main() {
  ...
  fs.writeFileSync(PURCHASES_PATH, JSON.stringify([], null, 2), "utf-8");

  console.log(`✅ Đã tạo ${accounts.length} acc mẫu (data/accounts.json) và ${users.length} tài khoản mặc định (data/users.json).`);
  console.log("   Admin: admin / admin123 (số dư 0đ)");
  console.log("   User:  user / user123 (số dư 0đ)");
  console.log("   Lịch sử mua hàng: data/purchases.json (rỗng)");
}
