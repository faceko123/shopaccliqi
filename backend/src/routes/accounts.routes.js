...
router.post("/bulk", requireAuth, requireAdmin, async (req, res) => {
  ...
  const acceptedItems = newItems.filter((item) => {
    if (existingImageUrls.has(item.image) || importedImageUrls.has(item.image)) {
      return false;
    }
    importedImageUrls.add(item.image);
    return true;
  });
  const skipped = newItems.length - acceptedItems.length;
  accounts.unshift(...acceptedItems);
  await db.saveAccounts(accounts);
  res.status(201).json({ created: acceptedItems.length, skipped });
});
