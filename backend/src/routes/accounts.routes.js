...
// Cùng một ảnh có thể được viết với hostname hoa/thường hoặc kèm #fragment.
// Chuẩn hóa trước khi so sánh để không thể lách kiểm tra trùng URL HTTPS.
...
router.post("/bulk", requireAuth, requireAdmin, async (req, res) => {
  ...
  const newItems = items.map((item, index) => {
    ...
    const image = normalizeHttpsImageUrl(item.image);
    ...
    if (!image) errors.push(`Dòng ${line}: URL ảnh phải bắt đầu bằng https:// hợp lệ.`);
  ...
  const existingImageUrls = new Set(
    accounts.map((account) => normalizeHttpsImageUrl(account.image)).filter(Boolean)
  );
  const importedImageUrls = new Set();
  newItems.forEach((item, index) => {
    const line = index + 2;
    if (existingImageUrls.has(item.image)) {
      errors.push(`Dòng ${line}: URL ảnh đã tồn tại trong danh sách acc.`);
    } else if (importedImageUrls.has(item.image)) {
      errors.push(`Dòng ${line}: URL ảnh bị trùng trong file CSV.`);
    }
    importedImageUrls.add(item.image);
  });
  if (errors.length > 0) {
    return res.status(409).json({ error: "Phát hiện URL ảnh trùng lặp.", details: errors.slice(0, 10) });
  }
...
router.post("/:id/purchase", requireAuth, async (req, res) => {
  const result = await db.purchaseAccount({
  ...
  if (result.status === "ACCOUNT_NOT_FOUND") return res.status(404).json({ error: "Không tìm thấy acc." });
  if (result.status === "USER_NOT_FOUND") return res.status(404).json({ error: "Không tìm thấy người dùng." });
  ...
  if (result.status === "INSUFFICIENT_BALANCE") {
    return res.status(400).json({
      error: `Số dư không đủ. Bạn cần ${result.account.price.toLocaleString("vi-VN")}đ nhưng ví chỉ có ${result.balance.toLocaleString("vi-VN")}đ. Vui lòng nạp thêm tiền.`,
      code: "INSUFFICIENT_BALANCE",
    });
  }
  const purchaseRecord = result.purchase;
